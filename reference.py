...
def generate_input(m: int, n: int, k: int, seed: int) -> input_t:
    ...
    # a = torch.ones((k, m), dtype=torch.bfloat16, device="cuda").to(torch.float8_e4m3fnuz)
    # a = (torch.arange(k * m, dtype=torch.bfloat16, device="cuda") % 8).reshape((k, m)).to(torch.float8_e4m3fnuz)
    # b = torch.ones((k, n), dtype=torch.bfloat16, device="cuda").to(torch.float8_e4m3fnuz)
    # b = (torch.arange(k * n, dtype=torch.bfloat16, device="cuda") % 8).reshape((k, n)).to(torch.float8_e4m3fnuz)
    # a = torch.eye(k, dtype=torch.bfloat16, device="cuda").to(torch.float8_e4m3fnuz)
    # b = torch.eye(k, dtype=torch.bfloat16, device="cuda").to(torch.float8_e4m3fnuz)
